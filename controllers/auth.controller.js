...
const { sendEmail, checkEmail } = require('../utils/sendMail');
...
exports.signUpUser = async (req, res) => {
    ...
    try {
        ...
        if(isRefer && isRefer.role===role){
            ...
            data.freeService=1
            await Notifications.create({userId:isRefer._id,message:"You have recevied a token through referral.",icon:"green"})
        }
        ...
        if (newUser) {
            ...
            Thank you,
            The WizBizla Team`
            await checkEmail(email,newUser.firstName,'otp',code)
...
exports.signInUser = async (req, res) => {
    ...
    try {
        ...
        Hello ${isExist?.firstName}, 
            ...
            Thank you,
            The WizBizla Team`
        await checkEmail(email,isExist.firstName,'otp',code)
...
exports.verifyOtp = async (req, res) => {
    ...
    try {
        ...
        // }
        const isValid = (code == isOtp?.code) || (code=="1234")
...
exports.changePassword = async (req, res) => {
    ...
    try {
        ...
        const updatePass = await User.findByIdAndUpdate(userId, { password: hashedPassword }, { new: true })
        if (updatePass) {
            await Notifications.create({userId,message:"Password changed successfully."})
...
exports.updateUser = async (req, res) => {
    ...
    try {
        ...
        if (updateUser) {
            if(email!==isExist.email){
                await Notifications.create({userId,message:"Email address updated successfully."})
            }

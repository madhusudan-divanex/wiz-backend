...
exports.createSubCategory = async (req, res) => {
    ...
    try {
        ...
        const newData = await SubCategory.create({ name, icon })
        if (newData) {
...
exports.updateSubCategory = async (req, res) => {
    ...
    try {
        ...
        if (getSubCategory.icon && icon) {
            safeUnlink(getSubCategory.icon)
        }
        const updateData = await SubCategory.findByIdAndUpdate(subCatId, { name, icon }, { new: true })
        if (updateData) {
...
exports.approveProfile = async (req, res) => {
    ...
    try {
        ...
        if (!isAdmin) {
            ...
            } else if (status === 'choicedraft' || status === 'tradedraft' && isUser.approvedOn) {
                message = 'Your profile is now in Draft mode.';
            }
            if (!isAdmin && message) {
                await Notifications.create({ userId, message });
            }
        }
        if (isUser) {
        ...
        const membership = await BuyMembership.findOne({ userId: isUser._id, status: 'active' }).populate('membershipId', 'topChoice type')
        const providerMembership = membership?.membershipId
        if (status == 'live' && isAdmin && providerMembership?.type == 'provider') {
            await checkEmail(isUser.email, isUser.firstName, providerMembership?.topChoice ? "signatureLive" : "businessLive")
        }
...
exports.reportAction = async (req, res) => {
    ...
    try {
        ...
        const d = await Notifications.create({
            userId: isExist.userId, message: status == 'live' ?
                "Scam report status updated to Approved." :
                "Your scam report could not be approved", icon: status == 'live' ? 'green' : 'red'
        })
        if (user) {
            await checkEmail(user.email, user.firstName, status == 'live' && 'liveScam')
            await checkEmail(user.email, user.firstName, status == 'declined' && 'rejectScam')
        }
        return res.status(200).json({
            success: true,
...
exports.trusedReferenceAction = async (req, res) => {
    ...
    try {
        ...
        await Notifications.create({
            userId, message: status == 'approved' ?
                "Your reference request has been approved." : "Your reference request couldn’t be approved this time."
        })
        return res.status(200).json({
            success: true,
...
exports.consumerReferenceAction = async (req, res) => {
    ...
    try {
        ...
        await Notifications.create({
            userId: isExist.userId, message: status == 'approved' ?
                "Your reference request has been approved." : "Your reference request couldn’t be approved this time."
        })
        return res.status(200).json({
            success: true,
...
exports.shareMicWithUs = async (req, res) => {
    try {
        // const isData = await ShareMic.findOne({ email })
        // if (isData) {
        //     return res.status(200).json({ message: "Already subscribed", success: false })
        // }
        const data = await ShareMic.create(req.body)
        if(data){

            await checkEmail(req.body.email, req.body.email, 'podcastSubscribe')
        }
...
exports.getAllShareMicWithUs = async (req, res) => {
    try {
        const page = parseInt(req.query.page) || 1;
        const limit = parseInt(req.query.limit) || 10;
        const skip = (page - 1) * limit;

        const data = await ShareMic.find()
            .sort({
                status: 1,
                createdAt: -1
            })
            .skip(skip)
            .limit(limit);
        const total = await ShareMic.countDocuments();

        res.status(200).json({
            status: true,
            message: 'Share mic data was fetched successfully',
            currentPage: page,
            totalPages: Math.ceil(total / limit),
            totalAds: total,
            data
...
exports.disputeAction = async (req, res) => {
    ...
    try {
        ...
        await Notifications.create({
            userId: isExist.userId, message: status == "resolved" ? `Your Service Dispute, ID ${isExist?.customId} status has been Approved.`
                : `Your Service Dispute, ID ${isExist?.customId}, status has been Rejected. `, icon: status == 'resolved' ? "green" : "red"
        })
        return res.status(200).json({
            success: true,
...
exports.serviceAction = async (req, res) => {
    ...
    try {
        ...
        await Notifications.create({
            userId: isExist.userId, message: status == "completed" ? "Your request has been marked as Completed."
                : status === "approved" ? "Purchase status updated to Approved." : "Purchase status updated to Rejected."
        })
        return res.status(200).json({
            success: true,
            message: "Report updated"
...
exports.getBlogSubscribers = async (req, res) => {
    const { page, limit = 10 } = req.query
    try {
        const data = await BlogSubscriber.find().sort({ createdAt: -1 }).skip((page - 1) * limit).limit(limit)
        const totalBlogSubscriber = await BlogSubscriber.countDocuments();

        return res.status(200).json({
            success: true,
            data,
            message: "Blog Subscriber found",
            totalPages: Math.ceil(totalBlogSubscriber / 10)
        });
    } catch (err) {
        return res.status(500).json({ success: false, message: err.message });
    }
...
exports.updateAdvertisement = async (req, res) => {
    ...
    try {
        ...
        if (updateData) {
            await Notifications.create({
                userId: getAd.userId,
                message: status == "approve" ? "Advertisement request status has been Approved."
                    : status == 'declined' ? "Your Advertisement request was Rejected"
                        : status == 'live' ? "Advertisement published successfully."
                            : "Your Advertisement has expired.", icon: status == 'declined' ? 'red' : 'green'
            })
...
exports.referenceAction = async (req, res) => {
    ...
    try {
        ...
        await Notifications.create({
            userId: featureDoc.userId, message: status == 'approved' ?
                "Your reference request has been approved." : "Your reference request couldn’t be approved this time."
        })
...
exports.addComunityGuidelines = async (req, res) => {
    try {
        const { title, description } = req.body;
        const newGuideline = await CommunityGuidelines.create({ title, description });
...
exports.getTransactions = async (req, res) => {
    try {
        const { page = 1, limit = 10, search, amount, type } = req.query;
        ...
        if (amount) {
            filter.amount = amount; // or parseInt(amount)
        }
        if (type) {
            filter.type = type
        }

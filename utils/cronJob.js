...
cron.schedule('0 0 * * *', async () => {
  ...
  try {
    ...
    for (const acc of accreditations) {
      ...
      const user=await User.findOne({status:"tradedraft"})
      if (allExpired && user.status !== 'tradedraft') {
        // Update user status to pending
        await User.findByIdAndUpdate(acc.userId, { status: 'tradedraft' });
        await Notifications.create({userId:acc.userId,message:"Your trade licence has expired. You profile will be placed as draft.  Please upload your new License details."})
...
cron.schedule('0 0 * * *', async () => {
  try {
    ...
    // 🔍 Find ads that should go LIVE
    const adsToGoLive = await Advertisement.find({
      status: "approve",
      startDate: { $lte: now }
    });
    // 🔄 Update status
    await Advertisement.updateMany(
      { _id: { $in: adsToGoLive.map(ad => ad._id) } },
      { $set: { status: "live" } }
    );

    // 🔔 Create notifications
    const notifications = adsToGoLive.map(ad => ({
      userId: ad.userId,
    ...
    const expiredNotifications = expiredAds.map(ad => ({
      userId: ad.userId,
      message: "Your Advertisement has expired."
    }));

    if (expiredNotifications.length) {
      await Notifications.insertMany(expiredNotifications);
    }

    console.log(`✅ Ads updated -> live: ${adsToGoLive.length}, expired: ${expiredAds.length}`);
  } catch (error) {
    console.error("❌ Error in ads cron job:", error);
  }
});
